...
use leto::{ApiError, error, error_and_message, wrap_error};
use serde_json::json;
use std::error::Error;
...
fn display_renders_code_and_message() {
    ...
    assert_eq!(e.to_string(), "leto error: c, message: m");

    let e = error("c");
    assert_eq!(e.to_string(), "leto error: c");
}
...
fn verbose_response_keeps_a_mixed_chain() {
    ...
    assert!(!json.contains("leto error:"), "internal prefix leaked: {json}");
}
