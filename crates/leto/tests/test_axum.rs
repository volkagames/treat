...
use leto::{ApiResponse, DEFAULT_ERROR_STATUS, error, response_get_api_error, success};
...
#[tokio::test]
async fn api_error_body_is_a_leto_envelope() {
    let resp = error("boom").into_response();
    let bytes = axum::body::to_bytes(resp.into_body(), usize::MAX)
        .await
        .expect("body should buffer");
    let decoded: ApiResponse = serde_json::from_slice(&bytes).expect("valid leto json");
