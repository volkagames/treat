...
fn assert_handler_fields(handler: &dyn leto::ApiErrorHandler) {
...
fn axum_stashes_the_error() {
    ...
    let handler = leto::response_get_api_error(&response).expect("handler in extensions");
...
fn actix_stashes_the_error() {
    ...
    let handler = leto::response_get_api_error_actix(&response).expect("handler in extensions");
    assert_handler_fields(handler.as_ref());
}
...
fn poem_stashes_the_error() {
    ...
    let handler = leto::response_get_api_error_poem(&response).expect("handler in extensions");
    assert_handler_fields(handler.as_ref());
}
...
async fn actix_extension_survives_the_service_pipeline() {
    ...
    async fn failing() -> Result<&'static str, leto::ApiError> {
    ...
    let handler = leto::response_get_api_error_actix(response.response()).expect("handler in extensions");
    assert_eq!(handler.code(), "boom");
    assert_eq!(handler.status(), 404);
}
...
fn an_unset_status_reports_has_status_false() {
    ...
    assert_eq!(handler.status(), leto::DEFAULT_ERROR_STATUS);
}
