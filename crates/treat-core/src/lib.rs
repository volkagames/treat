...
//! Core of the `treat` response envelope.
//!
...
//! depend on the `treat` facade crate, not this crate directly. Builds on
...
/// source chain. Use `treat::erris::report!`, `erris::Report`, `erris::Result`
...
// with a glob (that would collide) — reach them via `treat::extract_axum` /
// `treat::extract_actix`.
