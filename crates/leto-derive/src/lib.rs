...
///
/// The macros cannot hardcode `leto::`: a dependant may reach the items through
/// `leto-core` directly (no facade in the tree), rename the dependency, or shadow
/// the name with a local module. `proc-macro-crate` reads the caller's manifest
/// and returns the real name, falling back to `leto-core` when only the core
/// crate is present.
fn runtime_path() -> proc_macro2::TokenStream {
    use proc_macro_crate::{FoundCrate, crate_name};

    // The facade re-exports every item the macros need, so prefer it; a
    // core-only dependant gets `leto_core`.
    for candidate in ["leto", "leto-core"] {
...
/// ```
///
/// [`ErrorMessage`]: ../leto/struct.ErrorMessage.html
